...
@Component({
  ...
  styles: [
    `
      ...
      .section-card:nth-child(1) h3::before {
        content: '📊';
      }
      .section-card:nth-child(2) h3::before {
        content: '👤';
      }
      .section-card:nth-child(3) h3::before {
        content: '⭐';
      }
...
export class Dashboard implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const userProfile$ = this.authService.getUserProfile().pipe(
      catchError((err) => {
    ...
    const tests$ = this.orientationService.getUserTests().pipe(
      catchError((err) => {
        console.error('Error loading past tests:', err);
        this.error =
          (this.error ? this.error + ' ' : '') +
          "Impossible de charger l'historique de vos tests.";
        return of(null); // Continue even if tests fail
      })
    );

    forkJoin([userProfile$, tests$]).subscribe({
      next: ([userProfile, testsResponse]) => {
        this.user = userProfile;

        if (testsResponse) {
          // Gérer les deux formats possibles de réponse
          if (Array.isArray(testsResponse)) {
            this.pastTests = testsResponse;
          } else if (testsResponse && (testsResponse as any).results) {
            this.pastTests = (testsResponse as any).results;
          } else {
            this.pastTests = [];
          }
          ...
          this.activeTest =
            this.pastTests.find((test) => !test.is_completed) || null;
          this.completedTests = this.pastTests.filter(
            (test) => test.is_completed
          );
          this.latestTest =
            this.completedTests.length > 0 ? this.completedTests[0] : null;
        } else {
          this.pastTests = [];
      ...
      error: () => {
        // Errors are caught inside the streams, but as a fallback
        this.loading = false;
        this.cdr.detectChanges();
      },
    });
